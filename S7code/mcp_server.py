...
# Force UTF-8 on stdout/stderr so FastMCP's JSON-RPC output and any diagnostic
# writes don't hit Windows' default charmap codec when the page content
# contains non-cp1252 characters (e.g. → \u2192).
...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    # Redirect to os.devnull (not stderr) to avoid charmap encoding errors on
    # Windows when crawl4ai emits Unicode characters (e.g. → arrows).
    saved_fd = os.dup(1)
