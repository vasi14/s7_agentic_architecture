...
def observe(
    ...
    # ── Extend with in-run artifacts from history ─────────────────────────
    # fetch_url tool_outcome descriptors ("fetch_url(...) -> artifact art:X")
    # are semantically distant from free-form user queries, so they often
    # don't rank in the top-k vector hits.  Without this extension,
    # art_ids_in_order is empty, the safety-net never fires, and the agent
    # loops on search_knowledge (which returns nothing because index_document
    # was never called).  History is always authoritative for in-run work.
    _seen_art: set[str] = set(art_ids_in_order)
    _snapshot_extra: list[dict] = []
    _art_pos = sum(1 for h in hits[:12] if h.artifact_id)  # next i counter
    for _h in history:
        _aid = _h.get("artifact_id")
        if _aid and _aid not in _seen_art:
            art_ids_in_order.append(_aid)
            _seen_art.add(_aid)
            _snapshot_extra.append({
                "i": _art_pos,
                "kind": "tool_outcome",
                "descriptor": _h.get("result_descriptor", f"in-run artifact {_aid}"),
                "keywords": [_aid],
                "artifact_id": _aid,
            })
            _art_pos += 1
    ...
    snapshot_hits = _snapshot_hits(hits) + _snapshot_extra
    ...
    prompt = (
        ...
        f"{json.dumps(snapshot_hits, indent=2)}\n\n"
